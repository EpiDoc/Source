        ...
        <xsl:element name="body">
            ...
            <xsl:element name="div">
                ...
                <xsl:element name="p">
                    ...
                    <xsl:choose>
                    <xsl:when test="ancestor-or-self::*[@rend][1]/@rend != 'multipart'">
                    <xsl:for-each select="/*//tei:div">
                        <xsl:if
                            test="not(ancestor::tei:div[substring-after(@id, '-') = 'subsections'])">
                            <xsl:element name="a">
            ...
            <xsl:element name="div">
                ...
                <xsl:choose>
                    <xsl:when test="@rend='multipart'">
                        <xsl:apply-templates select="*[name() != 'div' and name() != 'head']" />
                        <xsl:element name="h2"><xsl:attribute name="id">subsections</xsl:attribute>Subsections:</xsl:element>
                        <xsl:element name="ul">
