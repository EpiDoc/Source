...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	<xsl:template match="lb"><xsl:if test="count(preceding::lb)>0"><xsl:if test="@type='worddiv'"><xsl:text> </xsl:text></xsl:if>/<xsl:if test="@type='worddiv'"><xsl:text> </xsl:text></xsl:if></xsl:if><xsl:element name="span">
