...
<xsl:stylesheet xmlns="http://www.w3.org/1999/XSL/TransformAlias"
    ...
    xmlns:axsl="http://www.w3.org/1999/XSL/TransformAlias" xmlns:str="http://exslt.org/strings"
    version="1.0">
    <!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
    ...
    <xsl:template name="copy-get-value-source">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <axsl:choose>
                    <axsl:when test="{source/xpath}">
                        <axsl:value-of select="normalize-space({source/xpath})" />
                    </axsl:when>
                    <xsl:if test="source/fallback">
                        <axsl:when test="{source/fallback}">
                            <axsl:value-of select="normalize-space({source/fallback})" />
                        </axsl:when>
                    </xsl:if>
                    <xsl:if test="default">
                        <axsl:otherwise>
    ...
    <xsl:template name="copy-get-value-sourceSplit">
        <xsl:choose>
            <xsl:when test="sourceSplit/xslt">
                <axsl:apply-templates select="{sourceSplit/xpath}" />
            </xsl:when>
            <xsl:otherwise>
                <axsl:choose>
                    <axsl:when test="{sourceSplit/xpath}">
                        <axsl:value-of select="normalize-space({sourceSplit/xpath})" />
                    </axsl:when>
                    <xsl:if test="sourceSplit/fallback">
                        <axsl:when test="{sourceSplit/fallback}">
                            <axsl:value-of select="normalize-space({sourceSplit/fallback})" />
                        </axsl:when>
                    </xsl:if>
                    <xsl:if test="default">
                        <axsl:otherwise>
    ...
    <xsl:template name="copy-get-value-sourceCombine">
        <xsl:if test="sourceCombine/prefix">
            ...
            <xsl:if test="sourceCombine/delimiter">
                <xsl:element name="axsl:if" namespace="http://www.w3.org/1999/XSL/Transform">
                    <xsl:attribute name="test">count(<xsl:value-of select="sourceCombine/xpath"
                             />)<xsl:if test="sourceCombine/fallback"> + count(<xsl:value-of
                                select="sourceCombine/fallback" />)</xsl:if> &gt; 0</xsl:attribute>
        ...
        <axsl:choose>
            <xsl:call-template name="copy-get-value-sourceCombine-option">
                <xsl:with-param name="optionval">
                    <xsl:value-of select="sourceCombine/xpath" />
                </xsl:with-param>
            </xsl:call-template>
            <xsl:if test="sourceCombine/fallback">
                <xsl:call-template name="copy-get-value-sourceCombine-option">
                    <xsl:with-param name="optionval">
                        <xsl:value-of select="sourceCombine/fallback" />
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:if>
            <xsl:if test="default">
    ...
    <xsl:template name="copy-enumerate">
        ...
        <xsl:choose>
            <xsl:when test="contains(sourceCombine/enumerate, 'parens')">
                <xsl:text>(</xsl:text>
                <xsl:choose>
                    <xsl:when test="$forcedval != ''">
                        <xsl:value-of select="$forcedval" />
                    </xsl:when>
                    <xsl:when test="$startval != ''">
                        <axsl:value-of select="position() + {$startval - 1}" />
                    </xsl:when>
                    <xsl:otherwise>
    ...
    <xsl:template name="copy-stringReplace">
        ...
        <axsl:variable name="replacement-0"><axsl:value-of select="&#x24;{$destname}"/></axsl:variable>
        <xsl:for-each select="stringReplace">
            <xsl:variable name="sRepIdxPrior">
                <xsl:value-of select="count(preceding-sibling::stringReplace)" />
            </xsl:variable>
            <xsl:variable name="sRepIdx">
                <xsl:value-of select="$sRepIdxPrior + 1"/>
            </xsl:variable>
            <axsl:variable name="replacement-{$sRepIdx}">
                <axsl:call-template name="stringReplace">
                    <axsl:with-param name="input"><axsl:value-of select="&#x24;replacement-{$sRepIdxPrior}" /></axsl:with-param>
                    <axsl:with-param name="find"><axsl:text><xsl:value-of select="find" /></axsl:text></axsl:with-param>
                    <axsl:with-param name="replace"><axsl:text><xsl:value-of select="replace" /></axsl:text></axsl:with-param>
                </axsl:call-template>
            </axsl:variable>
        </xsl:for-each>
        <axsl:value-of select="&#x24;replacement-{count(stringReplace)}"/>
    </xsl:template>
</xsl:stylesheet>
